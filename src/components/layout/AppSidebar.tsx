...
export function AppSidebar() {
  ...
  const NavItem = ({
    ...
    showOnMobile?: boolean;
      ...
      }} className={cn('flex items-center gap-3 rounded-lg px-3 py-2 transition-all hover:text-foreground hover:bg-accent', isActive(item.url) ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground')}>
  ...
  return <Sidebar collapsible="icon" className="border-r">
      <SidebarHeader className="border-b px-3 h-14 sm:h-16 flex items-center">
        <div className="gap-2 mx-[47px] flex-row flex items-center justify-between sm:gap-[19px] w-full">
          ...
          {isMobile && <Button variant="ghost" size="icon" className="h-8 w-8 -mr-1" onClick={() => setOpenMobile(false)}>
              <X className="h-4 w-4" />
